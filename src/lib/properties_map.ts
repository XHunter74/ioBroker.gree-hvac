...
export type PayloadConverterFunction = (
...
export class PropertyMapItem {
    ...
    isReadOnly(): boolean {
        const definition = JSON.parse(this.definition) as {
            common: {
                write?: boolean;
            };
        };
...
const propertiesMap: PropertyMapItem[] = [
    ...
    new PropertyMapItem(
        ...
        '{"type": "state", ' +
            ...
            '"desc": "Device date and time, format YYYY-MM-DD hh:mm",' +
            '"type": "string", ' +
            '"role": "date", ' +
            '"read": true, ' +
            '"write": true ' +
